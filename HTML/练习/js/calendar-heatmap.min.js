...
var calendarHeatmap = {
    ...
    drawGlobalOverview: function () {
        ...
        calendarHeatmap.items.selectAll(".item-block-year").data(t).enter().append("rect").attr("class", "item item-block-year").attr("width", function () {
            ...
            if (!calendarHeatmap.in_transition) {
                var e = "";
                e += "<div><span><strong>总在线时间:</strong></span>";
                var t = parseInt(a.total, 10),
                    n = Math.floor(t / 86400);
                n > 0 && (e += "<span>" + n + "天" + "</span></div>");
                var r = Math.floor((t - 86400 * n) / 3600);
                r > 0 && (e += n > 0 ? "<div><span></span><span>" + r + "小时" + "</span></div>" : "<span>" + r + "小时" + "</span></div>");
                var l = Math.floor((t - 86400 * n - 3600 * r) / 60);
                if (l > 0 && (e += n > 0 || r > 0 ? "<div><span></span><span>" + l + "分钟" + "</span></div>" : "<span>" + l + "分钟" + "</span></div>"), e += "<br />", a.summary.length <= 5)
                    for (var d = 0; d < a.summary.length; d++) e += "<div><span><strong>" + a.summary[d].name + "</strong></span>", e += "<span>" + calendarHeatmap.formatTime(a.summary[d].value) + "</span></div>";
                else {
                ...
                var c = calendarHeatmap.settings.tooltip_padding;
    ...
    drawYearOverview: function () {
        ...
        calendarHeatmap.items.selectAll(".item-circle").remove(), calendarHeatmap.items.selectAll(".item-circle").data(t).enter().append("rect").attr("class", "item item-circle").style("opacity", 0).attr("x", function (a) {
            ...
            var r = d3.scaleLinear().range(["#ffffff", a.color || calendarHeatmap.color || "#ff4500"]).domain([-.15 * n, n])
            ...
            if (!calendarHeatmap.in_transition) {
                ...
                var t = "";
                t += '<div class="header"><strong>' + moment(a.date).format("YYYY MMM Do, dddd") + '</strong></div>', t += "<div>在线时长 " + calendarHeatmap.formatTime(a.total) + "</div><br>";
                ...
                var o = calendarHeatmap.settings.item_size;
        ...
        calendarHeatmap.labels.selectAll(".label-month").remove(), calendarHeatmap.labels.selectAll(".label-month").data(o).enter().append("text").attr("class", "label label-month").attr("font-size", function () {
            ...
            return a.toLocaleDateString("zh-cn", {
                month: "short"
            })
        }).attr("x", function (a, e) {
        ...
        o.selectAll(".item-block-rect").data(function (a) {
            ...
            if (!calendarHeatmap.in_transition) {
                var e = new Date(d3.select(this.parentNode).attr("date")),
                    t = "";
                t += '<div class="header"><strong>' + a.name + "</strong></div><br>", t += "<div><strong>" + moment(e).format("YYYY MMM Do, dddd") + "</strong></div>", t += "<div>在线时长 " + calendarHeatmap.formatTime(a.value) + "</div>";
                ...
                var r = calendarHeatmap.settings.tooltip_padding;
    ...
    drawWeekOverview: function () {
        ...
        o.selectAll(".item-block-rect").data(function (a) {
            ...
            if (!calendarHeatmap.in_transition) {
                var e = new Date(d3.select(this.parentNode).attr("date")),
                    t = "";
                t += '<div class="header"><strong>' + a.name + "</strong></div><br>", t += "<div><strong>" + (a.value ? calendarHeatmap.formatTime(a.value) : "No time") + " tracked</strong></div>", t += "<div>在 " + moment(e).format("YYYY MMM Do, dddd") + "</div>";
                ...
                var i = calendarHeatmap.settings.tooltip_padding;
    ...
    drawDayOverview: function () {
        ...
        calendarHeatmap.items.selectAll(".item-block").remove(), calendarHeatmap.items.selectAll(".item-block").data(calendarHeatmap.selected.details).enter().append("rect").attr("class", "item item-block").attr("x", function (a) {
            ...
            if (!calendarHeatmap.in_transition) {
                var e = "";
                e += '<div class="header"><strong>' + a.name + "</strong><div><br>", e += "<div><strong>" + moment(a.date).format("YYYY MMM Do HH:mm, dddd") + "</strong></div>", e += "<div>在线时长 " + calendarHeatmap.formatTime(a.value) + "</div>";
                for (var n = 100 * a.value / 86400 + t(moment(a.date)); calendarHeatmap.settings.width - n < calendarHeatmap.settings.tooltip_width + 6 * calendarHeatmap.settings.tooltip_padding;) n -= 10;
                var r = calendarHeatmap.settings.tooltip_padding;
    ...
    drawButton: function () {
        ...
        a.append("text").attr("x", calendarHeatmap.settings.label_padding / 2.25).attr("y", calendarHeatmap.settings.label_padding / 2).attr("dy", function () {
            return Math.floor(calendarHeatmap.settings.width / 100) / 3
        }).attr("font-size", function () {
            return Math.floor(calendarHeatmap.settings.label_padding / 2) + "px"
    ...
    formatTime: function (a) {
        var e = Math.floor(a / 3600),
            ...
            n = "";
        return e > 0 && (n += e + "小时"), t > 0 && (n += t + "分钟"), 0 === e && 0 === t && (n = Math.round(a) + "秒"), n
    }
};